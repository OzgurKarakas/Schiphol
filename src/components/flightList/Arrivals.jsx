...
const Arrivals = ({ searchData }) => {
  ...
  const filteredArrivalFlightsEarlier = Object.values(data).flatMap((flights) =>
    flights.filter(
      (item) =>
        item.publicFlightState &&
        item.publicFlightState.flightStates &&
  ...
  const filteredArrivalFlightsLater = Object.values(data).flatMap((flights) =>
    flights.filter(
      (item) =>
        item.publicFlightState &&
        item.publicFlightState.flightStates &&
      ...
      <div className='space-y-2' >
        {showEarlier && (
          <div className='flex justify-center items-center'>
            {filteredArrivalFlightsEarlier.length === 0 ? (
              <p className='text-red-600'>No flights for previous dates were found.</p>
            ) : (
              filteredArrivalFlightsEarlier.map((item, index) => (
      ...
      <div>
        {showEarlierLater && (
          <div className='flex justify-center items-center m-2'>
            {filteredArrivalFlightsLater.length === 0 ? (
              <p className='text-red-600'>No flights for subsequent dates were found.</p>
            ) : (
              filteredArrivalFlightsLater.map((item, index) => (
                <button key={index} className='w-full bg-white p-[5px] md:p-[10px] border-none cursor-pointer flight-list-container'>
                  <div className='p-2'>
                    <div className='flex'>
                      <div className='w-[20%] md:w-[20%] text-[#141251] font-semibold flex flex-col justify-start custom-border'>
